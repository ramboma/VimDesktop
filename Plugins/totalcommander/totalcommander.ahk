...
;=======================================================
  ...
  Global tcconfig := Class_EasyINI(A_ScriptDir "\plugins\totalcommander\totalcommander.ini")
  Global TCPath := tcconfig.Config.TCPath
  Global TCINI  := tcconfig.Config.TCINI
	Global TCINIObj
...
TC_Usercmd_Load()
{
    global vim, obj_usercmd
    tccmd := A_ScriptDir "\plugins\totalcommander\totalcommander.ini"
    if not IsObject(obj_usercmd)
        obj_usercmd := []
    vim.mode("normal","TTOTAL_CMD")
    IniRead, var, %tccmd%, normal, , ""
    Loop, Parse, var, `n
    {
        key := RegExReplace(A_LoopField, "=.*$")
        action := RegExReplace(A_LoopField, "^[^=]*=")
        if RegExMatch(action, "i)^usercmd\|")
        {
	          vim.map(key, "TC_Usercmd_Do", "TTOTAL_CMD")
            obj_usercmd[key] := RegExReplace(A_LoopField, "i)^[^=]*=usercmd\|")
        }
        else
	          vim.map(key, action, "TTOTAL_CMD")
    }
}
TC_Usercmd_Do:
    TC_Usercmd_Do()
...
{
    global vim, obj_usercmd
    winObj := vim.GetWin("TTOTAL_CMD")
    cmd := obj_usercmd[winObj.KeyTemp]
...
azHistory2()
{
	...
	If Mod(LeftRight(),2)
	{
		If FileExist(f := tcconfig.redirect.LeftHistory)
	...
	Else
	{
		If FileExist(f := tcconfig.redirect.RightHistory)
